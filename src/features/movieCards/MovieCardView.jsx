...
const MovieCardView = (props) => {
      ...
      <>
        <NavLink onClick={selectMovie} to="/inspectMovie">
          ...
          <div className="imgWrapper">
            <img src={props.movie.posterURLs[500]}></img>
          </div>
        </NavLink>
        <div className="movieActionContainer">
          {props.list ? (
            <SpeedDial
              model={props.getItems(props.movie)}
              direction="right"
              buttonStyle={{
                background: "black",
                border: "none",
          ...
          {props.search ? (
            <Button
              className="addButton"
              icon="pi pi-plus"
              onClick={() => props.onPlusButtonClick(props.movie)}
            />
          ) : null}
        </div>
  ...
  return (
    <>
      <Toast ref={toast} />
      <div className="movieCard">{movieCard()}</div>
    </>
  );
};
